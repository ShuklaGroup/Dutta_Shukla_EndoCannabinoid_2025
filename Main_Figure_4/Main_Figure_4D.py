...
def kl_calculation(ref_state,mc):
    totdist = []
    query_filename = './CB2_assym_binding_' + mc + '_0_chv.npy'
    txx_query = 1/np.load(query_filename)

    ref_filename = './CB2_assym_binding_' + ref_state + '_0_chv.npy'
    ...
    np.save('./CB2_assym_binding_' + ref_state + '_' +  mc + '_kl_avg_score.npy',kl_avg)

    return kl_avg

def pdb_b_factor(r,q,j):
    f = open('./CB2_assym_binding_' + q +'_0.pdb','r')
    fh = open('./CB2_assym_binding_'+ r + '_' + q +'_b.pdb','w')
...
def vmd_writer(r,q,thickness,b_values):
    ...
    f = open('./CB2_assym_binding_' + r + '_' + q + '.vmd','w')
    f.write('mol new ./CB2_assym_binding_' + r + '_' + q + '_b.pdb type pdb first 0 last -1 \n')
...
def closest_atom_dist(state):
    ...
    traj = '../macrostate_rep_dcd/CB2_assym_binding_' + state + '_0.dcd'
...
if __name__=='__main__':
    ref_state = 'TM17'
    macrostate_names = ['TM56','B2','B1','TM17'] #state B1 represents the B$_{CB2}^{Pro}$; AB represents the I2$_{CB2}^{TM17}$; TM17 represents the I1$_{CB2}^{Pro}$

    for j,mc in enumerate(macrostate_names):
        if not os.path.exists('./important_files/CB2_assym_binding_' + mc + '_0_chv.npy'):
            closest_atom_dist(mc)

    a = np.load('./CB2_assym_binding_TM17_0_chv_ind.npy')
    indexs = np.unique(a)
    weights_per_residue = np.empty([len(indexs),len(macrostate_names)])

    for j,mc in enumerate(macrostate_names):
        kl_file = './CB2_assym_binding_' + ref_state + '_' +  mc + '_kl_avg_score.npy'
