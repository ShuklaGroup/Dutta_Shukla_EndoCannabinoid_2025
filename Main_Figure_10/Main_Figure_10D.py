...
def density_plot(ax):
    state_labels = ['A$_{CB2}^{Mem}$','U$_{CB2}^{Mem}$']
    ...
    plt.savefig('Main_Figure_10D.png',dpi=300)
    plt.close()


if __name__ == '__main__':
    macrostate_names = ['TM45','Bulk_position_TM45']
    ...
    for state in macrostate_names:
        ...
        for i in range(3):
            ...
            for j in range(1,1001):
                if os.path.exists('../pb_analysis/CB2_assym_binding_' + state + '_' + str(i) + '_' + str(j) + 'volumes.tabbed.txt'):
                    temp = np.loadtxt('../pb_analysis/CB2_assym_binding_' + state + '_' + str(i) + '_' + str(j) + 'volumes.tabbed.txt')
                    flag.append(temp[1])
