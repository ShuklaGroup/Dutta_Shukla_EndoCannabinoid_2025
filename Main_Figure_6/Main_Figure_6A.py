...
def bar_plot(stable_residues):
    colors = ['gold','paleturquoise','mediumpurple','salmon']
    contact_prob = pickle.load(open('CB2_state_wise_contact_probability.pkl','rb'))
    contact_energy = pickle.load(open('CB2_state_wise_contact_energy.pkl','rb'))
    ...
    ax1.plot([x1[0]+0.2+3*len(stable_residues)-0.5 for i in range(10)],np.linspace(0,1,10),'--',c='black')
    ...
    ax1.fill_between(np.arange(x1[0]+0.2+3*len(stable_residues)-0.5,round(x1[-1])+1.05,0.1),0,1,alpha=0.1,color=colors[3])
    ...
    plt.tight_layout()
    plt.savefig('./Main_Figure_6A.png',dpi=300)
    plt.close()


if __name__=='__main__':
    states = 4
    stable_residues = pickle.load(open('CB2_stable_residues.pkl','rb'))
