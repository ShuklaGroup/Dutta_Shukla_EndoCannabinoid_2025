...
def selection(extra_z,intra_z,totcom,mtM,ms):
    dtrajs_ref  = pickle.load(open('./CB2_frame_cluster_indices.pkl','rb'))
...
def scatter_plot(x_data,y_data,c_data,ms):
    colors = ['lightgreen','gold','salmon','mediumpurple','skyblue','paleturquoise','orange','black','violet']
    new_colors = [colors[i] for i in list(new_macrostate_index.keys())]
    cmap = mpl.colors.ListedColormap(new_colors[:ms])
    fig, axs = plt.subplots(1,1,figsize=(10,7))
    sc = plt.scatter(x_data,y_data,c=c_data,vmin =np.min(c_data),vmax = np.max(c_data),cmap=cmap,s=4)
    cbar = fig.colorbar(sc)
    cbar.ax.set_yticks(np.linspace(0,5,7)[:ms] + np.linspace(0,5,7)[1]/2,['A$_{CB2}^{Mem}$','U$_{CB2}^{Mem}$','I$_{CB2}^{TM17}$','I1$_{CB2}^{TM56}$','I2$_{CB2}^{TM56}$','B$_{CB2}^{Pro}$'],fontsize=20)
    ...
    #plt.grid()
    plt.tight_layout()
    plt.savefig('./Main_Figure_3D.png',transparent=False,dpi =500)
    plt.close()
...
if __name__ == '__main__':
    protein = 'CB2'
    ms =  6
    new_macrostate_index = {0:0,4:1,2:2,1:3,5:4,3:5}

    totdist = pickle.load(open('./CB2_lig_com_cor.pkl','rb'))
    weights = pickle.load(open('./CB2_msm_weight.pkl','rb'))
    mtM = pickle.load(open('./CB2_micro_to_macro_index.pkl','rb'))
